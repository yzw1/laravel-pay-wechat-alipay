...
//require_once 'AopEncrypt.php';
    ...
    {
        $this->toaccount = new AlipayFundTransToaccountTransferRequest();
        $this->encrypt   = new AopEncrypt();
    }
    ...
    */
    ...
    public function execute($request, $authToken = null, $appInfoAuthtoken = null) {
        ...
        if (strcasecmp($this->fileCharset, $this->postCharset)) {
            // writeLog("本地文件字符集编码与表单提交编码不一致，请务必设置成一样，属性名分别为postCharset!");
            Log::error("文件编码：[" . $this->fileCharset . "] 与表单提交编码：[" . $this->postCharset . "]两者不一致!");
        }
        $iv = null;
        if (!$this->checkEmpty($this->toaccount->getApiVersion())) {
            $iv = $this->toaccount->getApiVersion();
        } else {
            $iv = $this->apiVersion;
        }
        //组装系统参数
        ...
        $apiParams = $request->getApiParas();
        var_dump($apiParams);
        if (method_exists($request,"getNeedEncrypt") &&$this->toaccount->getNeedEncrypt()){

            $sysParams["encrypt_type"] = $this->encryptType;

            if ($this->checkEmpty($apiParams['biz_content'])) {

                Log::error(" api request Fail! The reason : encrypt request is not supperted!");
            }

            if ($this->checkEmpty($this->encryptKey) || $this->checkEmpty($this->encryptType)) {

                Log::error(" encryptType and encryptKey must not null! ");
            }

            if ("AES" != $this->encryptType) {

                Log::error("加密类型只支持AES");
            }

            // 执行加密
            $enCryptContent = $this->encrypt->encrypt($apiParams['biz_content'], $this->encryptKey);
            $apiParams['biz_content'] = $enCryptContent;
        ...
        try {
           ...
           Log::error($sysParams["method"], $requestUrl, "HTTP_ERROR_" . $e->getCode(), $e->getMessage());
            return false;
        }
        //解析AOP返回结果
        ...
        if (false === $respWellFormed) {
            Log::error($sysParams["method"], $requestUrl, "HTTP_RESPONSE_NOT_WELL_FORMED", $resp);
            return false;
        }
        // 验签
            ...
            if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
                ...
                if (!$checkResult) {
                    ...
                    if (strpos($signData->signSourceData, "\\/") > 0) {
                        ...
                        if (!$checkResult) {
                            Log::error("check sign Fail! [sign=" . $signData->sign . ", signSourceData=" . $signData->signSourceData . "]");
                        }
